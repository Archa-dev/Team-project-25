...
/* CSS styles for password change popup */
.password-popup {
    display: none;
    position: fixed;
    top: 50%;
...
<span class="password-close" onclick="closePasswordPopup()">&times;</span>
        <h2 id="adminName">EDIT ADMIN:</h2>
...
<script>
function openPasswordPopup(adminName) {
    document.getElementById("passwordPopup").style.display = "block";
    document.getElementById("adminName").textContent = "EDIT ADMIN: " + adminName; // Set the admin name in the popup
}
...
// Get all edit buttons and password popup
...
editButtons.forEach(button => {
    button.addEventListener('click', (event) => {
        ...
        // Get the admin name from the previous sibling element (box-label)
        const adminName = button.previousElementSibling.textContent.split(':')[1].trim();
        
        // Display password change popup with the admin name
        openPasswordPopup(adminName);
    });
});


// Close password change popup when clicking outside of it
...
// Prevent form submission and handle password change
